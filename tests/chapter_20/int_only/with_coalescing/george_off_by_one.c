...
int target(int a) {
    // Make sure we don't coalesce a into EDI; this would fail the George test
    ...
    // First we define a clique of 12 registers, one-twelve.
    // one interferes with a and has exactly twelve neighbors.
    // Once we prune a, we can prune one, then two-twelve.
    // To avoid spilling, one must go in EDI (two-twelve all conflict with EDI).
