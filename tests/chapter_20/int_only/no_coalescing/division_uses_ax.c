 ...
 * (There's no way to test these instructions separately.) Don't inspect
 * assembly, just validate behavior.
...
int main(void) {
    ...
    // movl %eax, %product
    // addl $4, %product
    // If we don't know eax is live, we'll coalesce product into EAX in second
    // round of coalescing.
