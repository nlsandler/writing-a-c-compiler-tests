    ...
    def get_test_for_path(cls, path: Path):
        if cls.TESTS is None:
            test_dict = {
                ...
                # possibly these rewrite instructions don't belong in chapter 21 test suite
                "spills_and_rewrites": lambda path: cls.make_spill_test(path, max_spilled_instructions=10, max_spilled_pseudos=3, extra_lib=Path("force_spill_lib.c")),
                "spills_rewrites_compare": lambda path: cls.make_spill_test(path, max_spilled_instructions=3, max_spilled_pseudos=1, extra_lib=Path("force_spill_lib.c")),
                "rewrite_large_multiply": lambda path: cls.make_spill_test(path, max_spilled_instructions=4, max_spilled_pseudos=1, extra_lib=Path("force_spill_lib.c")),
                "spill_movz_dst": lambda path: cls.make_spill_test(path, max_spilled_instructions=6, max_spilled_pseudos=3, extra_lib=Path("force_spill_lib.c")),
