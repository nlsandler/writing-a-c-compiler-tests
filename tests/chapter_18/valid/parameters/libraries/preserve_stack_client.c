// Make sure that when we copy values into the red zone during arg passing, we
// don't accidentally clobber the actual stack; this is a regression test for a
// bug in found in NQCC - copied into 0(rsp) instead of -8(rsp)
...
int check_struct(
    void);  // return 0 if static struct in lib is as expected, 1 otherwise
