...
// test 1: you can declare a function that accepts/returns incomplete struct types
// we don't define or use this function, we just need to validate
// that this declaration doesn't cause a compiler error
// TODO include test in appropriate folder where we define/call this after completing the type
struct never_used;
...
int test_block_scope_forward_decl(void) {
    ...
    struct s { int x; int y; }; // complete the type
...
// test 5: you can dereference a pointer to an incomplete var, then take its address
